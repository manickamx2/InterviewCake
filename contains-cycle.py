...
def contains_cycle(first_node):
    if first_node:
        first_node.visited = True # mark as visited
        next_node = first_node.next
    else:
        return False

    if next_node: # if the next node is not None
        if next_node.visited: # has it been visited?
            return True
        else:
            return contains_cycle(next_node) # recur on next node
    else: # we're at the end of the list, no cycles detected
        return False
...
class Test(unittest.TestCase):
    ...
    class LinkedListNode(object):
        ...
        def __init__(self, value, next=None):
            self.value = value
            self.next  = next
            self.visited = False # add an attribute to linked list nodes indicating visited or not
